num_str = input('Введите последовательность чисел через пробел: ') # получаем последовательность
...
def up_sort(array): # определяем функцию для сортировки списка по возрастанию
...
num_list_sort = up_sort(num_list)
print('Список отсортированный по возрастанию элементов: ', num_list_sort)
def pos_search(a, array): # определяем функцию для поиска позиции
...
item = pos_search(num, num_list_sort)

if num > num_list_sort[-1]:  # добавляем проверку что введенное число соответствует условию
    print(f'Ввденное число {num} больше любого числа в последовательности {num_list}')
elif num < num_list_sort[0]:
    print(f'Ввденное число {num} меньше любого числа в последовательности {num_list}')
else:
